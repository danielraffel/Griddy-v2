...
find . -type f \( -name "*.cpp" -o -name "*.h" -o -name "*.cmake" -o -name "*.txt" -o -name "*.md" -o -name ".env*" \) -exec sed -i '' \
    -e "s/Griddy/$PLUGIN_NAME/g" \
    -e "s/Griddy/$CLASS_NAME/g" \
    -e "s/griddy-v2/$PROJECT_FOLDER/g" \
    -e "s/com.generouscorp.griddy-v2/$PROJECT_BUNDLE_ID/g" \
    -e "s/Generous Corp/$DEVELOPER_NAME/g" \
    -e "s/Generous Corp/$PLUGIN_MANUFACTURER/g" \
    -e "s/generouscorp/$NAMESPACE/g" \
    -e "s/GRDY/$PLUGIN_CODE/g" \
    -e "s/GECO/$PLUGIN_MANUFACTURER_CODE/g" \
    {} \; 2>/dev/null || true
